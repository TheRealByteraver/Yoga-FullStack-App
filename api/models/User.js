...
const BIOGRAPHY_MAX_LENGTH = 1024;
...
module.exports = (sequelize) => {
  ...
  User.init({
    ...
    // The string contains zero or more "roles" separated by a space. Possible roles are:
    //     'superadmin', 'admin', 'coach', 'athlete' (full lowercase)
    roles: {
    ...
    dateOfBirth: {
      ...
      type: DataTypes.INTEGER, // in Unix epoch
      allowNull: false,
      validate: {
        notNull: {
        ...
        notEmpty: {
          msg: 'Please provide the date on which you were born'
        },
      }
    },
    biologicalSex: {
      type: DataTypes.STRING, // 'M' or 'F'
      allowNull: true,
    },
    phone: {
      type: DataTypes.STRING,
    ...
    biography: {
      type: DataTypes.STRING(BIOGRAPHY_MAX_LENGTH),
      allowNull: true,
    },
    avatarUrl: {
      type: DataTypes.STRING,
